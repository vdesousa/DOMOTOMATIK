...
$email = $password = "";
$password_db=$pdo->query('SELECT password FROM administrateur');
$showPassword = $password_db->fetch();
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($email_error) && empty($password_error)){
        ...
        $sql = "SELECT email, password FROM administrateur WHERE email = :email";
        ...
        if($state = $pdo->prepare($sql)){
            ...
            if($state->execute()){
                ...
                if($state->rowCount() == 1){
                    if($row = $state->fetch()){
                        if(trim($_POST['password']) == $showPassword['password']){
